...
with tab_signal:
    ...
    if sig_data.get("error"):
        ...
        with col_breakdown:
            ...
            sources = [
                ("📊 Technical (RSI + MACD)", _signal_pill(tech_s)),
                ("📰 News Sentiment (FinBERT)", _signal_pill(news_s)),
                ("🎲 Polymarket Crowd", _score_pill(p_score, p_label)),
            ]

            for src_label, pill_html in sources:
                st.markdown(
                    f"<div class='reason-item'>"
                    f"<span style='flex:1;color:var(--text-primary);'>{src_label}</span>"
                    f"{pill_html}"
                    f"</div>",
                    unsafe_allow_html=True,
                )
            ...
            if reasons:
                st.markdown(
                    "<div class='section-label' style='margin-top:20px;'>Reasoning</div>",
                    unsafe_allow_html=True,
                )
                for r in reasons:
                    st.markdown(
                        f"<div class='reason-item'>"
                        f"<span style='color:#4a90e8;'>›</span> {r}"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
